    ...
    <div className="mt-3">
      ...
      <div className="relative mt-1 h-3">
        {[
          ...
          <span
            key={t.label}
            className="absolute -translate-x-1/2 text-[10px] text-[var(--dtg-text-muted)] tabular-nums"
            style={{ left: `${t.at}%` }}
          >
            {t.at}
          </span>
...
function IndexBPanel({ assessment }: { assessment: Assessment }) {
  if (assessment.indexBAvailable) {
    return (
      <div className="flex items-start gap-2 rounded-lg border border-border bg-muted/40 p-3 text-sm">
        <SunDim className="mt-0.5 size-4 shrink-0 text-[var(--dtg-text-muted)]" aria-hidden />
  ...
  return (
    <div className="flex items-start gap-2 rounded-lg border border-dashed border-border bg-muted/30 p-3 text-sm">
      <Moon className="mt-0.5 size-4 shrink-0 text-[var(--dtg-text-muted)]" aria-hidden />
      <p className="text-[var(--dtg-text-muted)]">
  ...
  return (
    <div>
      <div className="mb-2 flex items-baseline justify-between">
        <h4 className="text-xs font-semibold uppercase tracking-wider text-[var(--dtg-text-muted)]">
          Component breakdown
        </h4>
        <span className="text-xs text-[var(--dtg-text-muted)] tabular-nums">
            ...
            return (
              <tr key={c.component} className="border-b border-border/60 last:border-0">
                <td className="py-2 pr-3 align-top">
                  <div className="font-medium text-[var(--dtg-text-muted)]">
                    {COMPONENT_LABEL[c.component] ?? c.component}
                  </div>
                  <div className="text-xs text-[var(--dtg-text-muted)]">{c.detail}</div>
                  {!c.available && (
                ...
                <td className="w-14 py-2 pl-3 text-right text-[var(--dtg-text-muted)] align-top tabular-nums">
                  {c.points}
                  <span className="text-[var(--dtg-text-muted)]">/{c.max}</span>
                </td>
              </tr>
            );
          })}
  ...
  if (!data) {
    return (
      <Card className={loading ? 'opacity-60' : ''}>
        ...
        <CardContent>
          <p className="text-sm text-[var(--dtg-text-muted)]">No data.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <Card className={loading ? 'opacity-60 transition-opacity' : 'transition-opacity'}>
      <CardHeader className="border-b">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <CardTitle className="text-base text-[var(--dtg-text-muted)] font-semibold">Fog status</CardTitle>
      ...
      <CardContent className="space-y-5">
        <div>
          ...
          <p className="mt-2 text-sm text-[var(--dtg-text-muted)]">
        ...
        {score !== null && score !== undefined ? (
          <div>
            <div className="flex items-baseline gap-2">
              ...
              <span
                className="text-4xl text-[var(--dtg-text-muted)] font-semibold leading-none"
                style={{ color: style.token }}
              >
                {score}
              </span>
              <span className="text-sm text-[var(--dtg-text-muted)]">/ 100 · index A</span>
            </div>
        ...
        {assessment && assessment.gates.length > 0 && (
          <div className="rounded-lg border border-[var(--status-good)]/30 bg-[var(--status-good)]/8 p-3">
            ...
            <ul className="mt-1.5 space-y-0.5 text-sm text-[var(--dtg-text-muted)]">
            ...
            <p className="mt-1.5 text-xs text-[var(--dtg-text-muted)]">
        ...
        {assessment && (
          <dl className="grid grid-cols-2 gap-x-4 gap-y-2 border-t pt-3 text-xs text-[var(--dtg-text-muted)] sm:grid-cols-4">
            {[
              ...
              <div key={label}>
                <dt className="text-[var(--dtg-text-muted)]">{label}</dt>
