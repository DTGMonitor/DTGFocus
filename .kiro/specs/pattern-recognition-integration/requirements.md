...
13. THE Auto_Fill_Mapper SHALL merge the pattern-recognition-derived values with the precursors record's `initialValues`, where `Location` and `alarmRegions` from the precursors record take precedence and all other fields are filled from the pattern-recognition output.
